...
namespace InCoreLib.Domain.Allocations.Database.Extensions
{
    public static class TypeExtensions
    {
        ...
        public static IEnumerable<string> GetPrimaryKeyNames(this Type type, ITrackedContext context)
        {
        ...
        public static string GetTableName(this Type entityType, ITrackedContext context)
        {
            string key = entityType.FullName;
            string tableName = GetFromCache(TableNameCache, key, k => TableNameFactory(entityType, context));
        ...
        private static IEnumerable<string> PrimaryKeyNamesFactory(Type type, ITrackedContext context)
        {
        ...
        private static string TableNameFactory(Type entityType, ITrackedContext context)
        {
            var tableAttribute =
        ...
        {
            TVal returnVal = default(TVal);
            lock (dictionary)
            {
                try
                {
                    returnVal = dictionary.GetOrAdd(key, valueFactory);
                }
                catch (Exception)
                {
                }

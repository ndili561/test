namespace Incomm.Allocations.DAL.Migrations.SQL
{
   public partial class SqlProgrammability    {
        ...
        public static string Alter_PropertyMatchForApplicationId
        {
            get { return @"ALTER PROCEDURE [dbo].[PropertyMatchForApplicationId]
    (
    ...
    BEGIN
        ...
        SET NOCOUNT ON;
          DECLARE @AGE INT = (DATEDIFF(hour,(SELECT TOP 1 DateOfBirth FROM SearchContacts WHERE ApplicationId = @ApplicationId AND ContactTypeName = 'Applicant'),GETDATE())/8766 )
        ...
        ;WITH    CTEInterests
             ...
             FROM     dbo.VBLApplications AS CA 
						...
						--and  p.PropertyCode ='853906511' 
                                ...
                                AND ((RPAR.ApplicationID IS NULL AND CAST(Replace(p.AgeCriteria,'.00','') AS INT) <= @Age ) -- If the applicant's age is already 55 or 65 he automatically qualify for the property
											OR ISNULL(Replace(p.AgeCriteria,'.00',''),'NA')= ISNULL((SELECT REPLACE(Name,'+','') FROM AgeRestrictions WHERE AgeRestrictionId =  RPAR.AgeRestrictionId),'AN')
									)
                                AND (RPAD.ApplicationId IS NULL 
										OR(RPad.AdaptationId = 1 and ISNULL(p.IsWheelChairAdapted,0)= 1) 
										OR((ISNULL(p.HasRampedAccess,0) = 1 
												OR ISNULL(p.HasStairlift,0) = 1 
												OR ISNULL(p.HasWalkInShower,0) = 1 
												OR ISNULL(p.HasStepInShower,0) = 1)
										AND RPAD.AdaptationID = 2)
									)
								 AND  (
									(CAST(ISNULL(p.NumStepsToAccess , 0) AS int) = 0 AND PRD.NumberOfSteps in (1,2,3,4,5)) OR
                                    (CAST(ISNULL(p.NumStepsToAccess , 0) AS int) < 3 AND PRD.NumberOfSteps IN(2,3,4,5)) OR
                                    (CAST(ISNULL(p.NumStepsToAccess , 0) AS int) < 10 AND PRD.NumberOfSteps in (3,4,5)) OR
                                    (CAST(ISNULL(p.NumStepsToAccess , 0) AS int) < 20 AND PRD.NumberOfSteps in (4,5)) OR
                                    (CAST(ISNULL(p.NumStepsToAccess , 0) AS int) > 19 AND PRD.NumberOfSteps = 5) 
									)
									AND(
									(PRD.HighRise = 1) OR
                                    (PRD.HighRise  IS NULL) OR
                                    (PRD.HighRise = 0 AND p.IsHighRise = 0)
									)
 ...
 UNION
               ( SELECT			0													AS VoidId,
                                ...
                                p.IsLevelAccessProperty								AS LevelAccessProperty,
                                p.HasStairlift										AS Stairlift,
                                CAST(p.AgeCriteria AS VARCHAR(100))					AS AgeRestriction,
                                CASE p.CatDog WHEN 0 THEN 'N' ELSE 'Y'  END			AS Pets,
                                CAST(ISNULL(p.NumStepsToAccess , 0)	as int)			AS NumberOfSteps,
                                p.HasLift											AS Lift,
                                p.CommEnt											AS CommunalEntrance,
                                p.HighRise											AS HighRise,
                                CAST(p.rent AS NUMERIC)								AS Rent ,
                                CAST(p.ServiceCharge AS NUMERIC)					AS ServiceCharges,
								p.RSL												AS LandLord,
                   ...
                   FROM     dbo.VBLApplications AS CA 
						...
						WHERE    ca.ApplicationId = @ApplicationID
                                ...
                                AND ((RPAR.ApplicationID IS NULL AND (@Age IS NULL OR CAST(Replace(p.AgeCriteria,'.00','') AS INT) <= @Age))
                               ...
                               AND Ignore.ApplicationID IS NULL
							   AND p.status = 'Void'
							   AND P.PropertyCode NOT IN (SELECT PropertyCode FROM dbo.VBLCustomerInterests WHERE CustomerInterestStatusId IN (1,10))
 )
                      )    
